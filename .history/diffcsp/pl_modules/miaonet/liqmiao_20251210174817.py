...
# time infomation is embedded in readout layer


class LiqMiaoNet(CrystalDiffusion):
    ...
    def __init__(
        ...
        self.readout_layer = ReadoutLayer(
            ...
            target_way={"energy_p": 0},
            target_channel={"energy_p": 1},
            # target_way={"cart_diff": 1, "cell_stress": 0},
            # target_channel={"cart_diff": 1, "cell_stress": 9},
    ...
    def get_diffusion(
        ...
        # cell: [n_batch, 3, 3]
        output_tensors["cart_diff"].squeeze()
        inv_cell = batch_data['inv_cell'].repeat_interleave(batch_data["n_atoms"], dim=0)
        frac_diff = torch.bmm(output_tensors["cart_diff"].view(-1, 1, 3), inv_cell).squeeze(1)

        cell_stress = output_tensors["cell_stress"]
        # cell_stress = 0.5 * (cell_stress + cell_stress.transpose(1, 2))
        cell_stress[:, [0, 1, 2], [0, 1, 2]] += output_tensors["stress_diag"][:, None]

        # cell_stress = _scatter_mean(cell_stress, batch_data["batch"])
        # cell_diff = torch.bmm(batch_data['cell'], cell_stress)
        cell_diff = _scatter_mean(cell_stress, batch_data["batch"])
        # cell_diff = torch.linalg.inv(cell_diff)
        # cell_diff[:, [0, 1, 2], [0, 1, 2]] = cell_diff[:, [0, 1, 2], [0, 1, 2]] - 1
        return frac_diff, cell_diff
