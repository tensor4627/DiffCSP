...
from diffcsp.pl_modules.diff_utils import soften_coordinates_piecewise
MAX_ATOMIC_NUM=100
...
class JVPCSPNet(nn.Module):
    ...
    def single_forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph):
    ...
    def forward(self, x_tot, r,t):
        mask = self.jvp_mask
        lattices_item = x_tot.shape[0]-mask["num_atoms"][0]
        lattices = x_tot[:lattices_item,:].view(-1,3,3)
        frac_coords = x_tot[lattices_item:,:]
        lattice_out,coord_out = self.single_forward(t=t+r,atom_types=mask["atom_types"],frac_coords=frac_coords,lattices=lattices,num_atoms=mask["num_atoms"],node2graph=mask["node2graph"])
        total = torch.cat([lattice_out.view(-1,3),coord_out],dim=0)
        return total

    def get_jvp_info(self,mask):
        self.jvp_mask = mask

class TimeCSPNet(nn.Module):
    ...
    def __init__(
        ...
        if self.smooth:
            ...
            self.node_embedding = nn.Embedding(max_atoms, hidden_dim)
        self.atom_latent_emb = nn.Linear(hidden_dim + latent_dim, hidden_dim)
        self.time_embedding = SinusoidsEmbedding(time_dim)
        if act_fn == 'silu':
            self.act_fn = nn.SiLU()
        if dis_emb == 'sin':
            self.dis_emb = SinusoidsEmbedding(n_frequencies = num_freqs)
    ...
    def single_forward(self, time, atom_types, frac_coords, lattices, num_atoms, node2graph):
    ...
    def forward(self, x_tot, r,t):
        ...
        lattices = x_tot[:lattices_item,:].view(-1,3,3)
        frac_coords = x_tot[lattices_item:,:]
        double_time = torch.cat([r.unsqueeze(0),t.unsuqeeze(0)],dim=0)
        lattice_out,coord_out = self.single_forward(t=double_time.view(2,-1),atom_types=mask["atom_types"],frac_coords=frac_coords,lattices=lattices,num_atoms=mask["num_atoms"],node2graph=mask["node2graph"])
        total = torch.cat([lattice_out.view(-1,3),coord_out],dim=0)
        return total
...
class SoftNoTimeCSPNet(nn.Module):
