...
class CSPEqM(BaseModule):
    ...
    def forward(self, batch):
        ...
        pred_l_velocity, pred_x_velocity = self.decoder(time_emb, batch.atom_types, input_frac_coords, input_lattice, batch.num_atoms, batch.batch)
        c_gamma = self.mild_target_trunc(times)
        target_l = (lattices-rand_l)*c_gamma.view(-1,1,1)
        target_f = self.wrapped_distance_vector(rand_x,frac_coords)*c_gamma.repeat_interleave(batch.num_atoms)[:, None]
        loss_lattice = F.mse_loss(pred_l_velocity, target_l)
        loss_coord = F.mse_loss(pred_x_velocity, target_f)
